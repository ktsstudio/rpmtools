...
bin="/usr/bin/${name}"
opts="-E --pidfile=${pidfile} -c 5 -l INFO --logfile=/var/log/${name}/celeryd.log --uid=$(id -u ${name}) --gid=$(id -g ${name}) --workdir=/opt/${name}/src"
...
stop() {
	...
	${bin} celeryd_multi stop celery --pidfile=${pidfile}
	RETVAL=$?
	echo
